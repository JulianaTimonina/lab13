...
func makeDecision(
	ctx context.Context,
	nc *nats.Conn,
	income common.IncomeAnalysis,
	risk common.RiskAssessment,
	...
	for range ticker.C {
		...
		if err == nil {
			...
			if pending > 10 {
				...
				log.Printf(
					"Need to scale UP income-analyzer, pending=%d",
					pending,
				)
				...
				log.Printf(
					"Can scale DOWN income-analyzer, pending=%d",
					pending,
				)
			}
		}
		...
		if err == nil {
			...
			if info.NumPending > 10 {
				...
				log.Printf(
					"Need to scale UP risk-evaluator, pending=%d",
					info.NumPending,
				)
				...
				log.Printf(
					"Can scale DOWN risk-evaluator, pending=%d",
					info.NumPending,
				)
			}
		}
